...
btns.forEach(btn => {
  ...
  btn.addEventListener('click', () => {
    ...
    // обновляем span-элементы для вопроса и ответа
    answerEls[questionClass].answer.textContent = null; // очищаем содержимое span-элемента
    const br = document.createElement("br"); // создаем новый элемент <br>
    answerEls[questionClass].answer.appendChild(document.createTextNode(" - " + answerText)); // добавляем текст ответа
    answerEls[questionClass].answer.appendChild(br); // добавляем элемент <br>

    // добавляем или заменяем span-элементы в блок ответов
    if (answerBox.contains(answerEls[questionClass].question)) {
      answerBox.replaceChild(answerEls[questionClass].answer, answerEls[questionClass].answer);
    } else {
      answerBox.appendChild(answerEls[questionClass].question);
      answerBox.appendChild(answerEls[questionClass].answer);
    }
  });
});
