...
// получаем элемент блока .answers
...
btns.forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    // находим элементы вопроса и ответа для соответствующего вопроса
    const questionClass = "вопрос-" + question;
    const answerEls = answerBox.querySelectorAll(`.${questionClass}`);
    const questionEl = answerEls[0];
    const answerEl = answerEls[1];

    // создаем новые элементы вопроса и ответа
    const newQuestionEl = document.createElement("span");
    newQuestionEl.classList.add(questionClass);
    newQuestionEl.textContent = questionText + question;
    const newAnswerEl = document.createElement("span");
    newAnswerEl.textContent = answerText;

    // проверяем, есть ли уже элемент ответа для этого вопроса
    if (answerEls.length > 0) {
      // если есть, то заменяем его
      if (newQuestionEl.textContent !== questionEl.textContent) {
        answerBox.replaceChild(newQuestionEl, questionEl);
      }
      answerBox.replaceChild(newAnswerEl, answerEl);
    } else {
      // если нет, то создаем новый элемент
      answerBox.appendChild(newQuestionEl);
      answerBox.appendChild(newAnswerEl);
    }
  });
});
